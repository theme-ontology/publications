...
for model_name,model in models:
    ...
    for bm in sorted(list(BM.keys())):
        ...
        for story in storyset:
            ...
            result_list=[episodes[s[0]] for s in similars if story!=episodes[s[0]]]
            #print(len(similars),len(result_list))
            top10set=set([episodes[s[0]] for s in similars[:11]])
            top10set=set(result_list[:10])
            
            P_AT_10=(len(storyset.intersection(top10set))-0)/10
            sum_P_AT_10+=P_AT_10
            
            #AP calculation
            AP_sum=0.0
            for s in storyset-set([story]):
                k=result_list.index(s)+1 # attention k is a zero-based index
                precision_at_k=len(storyset.intersection(set(result_list[:k])))/k
                AP_sum+=precision_at_k
            sum_MAP+=AP_sum/(len(storyset)-1)
        MAP=sum_MAP/len(storyset)
        summation_MAP+=MAP
        print(name,MAP,sum_P_AT_10/len(storyset))
        summation_P_10+=sum_P_AT_10/len(storyset)
        n+=1
    print(n,len(BM))
    print("Average",summation_MAP/n,summation_P_10/n)
    print()
